...
describe("bashEnv bridge", () => {
  test("exported vars from the snippet land in nu's $env", async () => {
  ...
  test("loadBashEnv returns only changed vars", async () => {
    if (!(await bashRuntimeAvailable())) {
      console.warn("skipping bashEnv test — no bash runtime detected")
      return
    }
...
describe("audit regressions", () => {
  test("noCapture preserves bashEnv", async () => {
  ...
  test("bashEnv ignores prelude stdout noise", async () => {
    if (!(await bashRuntimeAvailable())) return
  ...
  test("bashEnv preserves multi-line values", async () => {
    if (!(await bashRuntimeAvailable())) return
    const result = await loadBashEnv(
      "export NC_MULTI=$'line-one\\nline-two\\nline-three'",
    )
    expect(result.vars.NC_MULTI).toBe("line-one\nline-two\nline-three")
  }, 20_000)
  ...
  test("bashEnv: empty script is a no-op (no subprocess fired)", async () => {
    ...
    process.env.NUSHELL_MCP_BASH_PATH = "/definitely/not/a/real/bash/binary"
    try {
...
describe("second-pass audit", () => {
  ...
  test("runPipeline propagates timeoutMs to the bash bridge", async () => {
    if (!(await bashRuntimeAvailable())) return
  ...
  test("killAll cancels in-flight nu processes", async () => {
...
describe("cycle 2 audit regressions", () => {
  ...
  test("NUSHELL_MCP_BASH_PATH to nonexistent path throws instead of falling through", async () => {
    const prevOverride = process.env.NUSHELL_MCP_BASH_PATH
  ...
  test("runPipeline leaves PipelineResult.bashRunner undefined when no bashEnv", async () => {
...
describe("MCP server wiring", () => {
  test("server initializes over stdio and lists its twelve tools", async () => {
    ...
    try {
      ...
      expect(searchResult.result.structuredContent.kind).toBe("commands")
  ...
  test("nu_repl_spawn / nu_repl_list / nu_repl_kill / nu_repl_nuke", async () => {
    ...
    try {
      ...
      expect((w2.result.structuredContent?.output as string) ?? "").toContain(
        "42",
      )
      ...
      expect(
        (abortRes.result.structuredContent?.aborted as number) ?? 0,
      ...
      expect((iso1.result.structuredContent?.output as string) ?? "").toContain(
        "1",
      )
      expect((iso2.result.structuredContent?.output as string) ?? "").toContain(
        "2",
      )
  ...
  test("nu_exec: [bashEnv runner: ...] label appears in MCP content text", async () => {
    if (!(await bashRuntimeAvailable())) {
      console.warn(
