...
import { active, addActive, removeActive } from "./active.js"
import {
...
/**
Absolute path to the bundled Nushell configuration.
Override with `NUSHELL_MCP_CONFIG_PATH`.
*/
export const CONFIG_PATH: string =
  process.env.NUSHELL_MCP_CONFIG_PATH ??
  join(resolveServerRoot(), "config", "config.nu")

/** Bail early if config does not resolve. */
...
/** Nushell invocation constant to ensure config is always used */
const NU_COMMAND = [NU_PATH, "--config", CONFIG_PATH] as const
...
export async function runRaw(
  ...
  if (opts.input === undefined) return spawnNu(["--commands", code], opts)
...
export async function runPipeline(
  ...
  await Bun.write(scriptPath, buildScript({ pipeline, hasInput, capture }))
